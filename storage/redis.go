...
func (redisClient *RedisClient) Check() (string, error) {
	return redisClient.client.Ping().Result()
}

func (redisClient *RedisClient) BgSave() (string, error) {
	return redisClient.client.BgSave().Result()
}

func (redisClient *RedisClient) GetBlockFees(height int64, blockType string) (*big.Int, error) {
	option := redis.ZRangeByScore{Min: strconv.FormatInt(height, 10), Max: strconv.FormatInt(height, 10)}
	cmd := redisClient.client.ZRangeByScoreWithScores(redisClient.formatKey("blocks", blockType), option)
	if cmd.Err() != nil {
		return nil, cmd.Err()
	}
...
func (redisClient *RedisClient) formatKey(args ...interface{}) string {
	return join(redisClient.prefix, join(args...))
}
